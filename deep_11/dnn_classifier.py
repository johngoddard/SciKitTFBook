...
class DNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _dnn(self, inputs):
        ...
        for layer in range(self.n_hidden_layers):
            ...
            inputs = tf.layers.dense(inputs, self.n_nuerons, kernel_initializer=self.initializer,
    ...
    def fit(self, X, y, n_epochs=100, X_valid=None, y_valid=None):
        ...
        with self._session.as_default() as sess:
            ...
            for epoch in range(n_epochs):
                ...
                for rnd_idxes in np.array_split(rnd_idx, len(X) // self.batch_size):
                    X_batch, y_batch = X[rnd_idxes], y[rnd_idxes]
                    feed_dict = {self._X: X_batch, self._y: y_batch}
                    if self._training is not None:
                        feed_dict[self._training] = True
                    ops = [self._training_op]
                    if extra_update_ops:
                        ops.append(extra_update_ops)
            ...
            # restore best_params when applicable
            if best_params:
